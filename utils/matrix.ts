...
export const BASIC_DIRS: DIR[] = [UP, RIGHT, DOWN, LEFT];
...
export class Matrix<T> {
  ...
  public move = (
    ...
    return [c[0] + step * vDir, c[1] + step * hDir];
  };
  ...
  public exists = (c: Coords): boolean => {
    return (
      isBetween(0, c[0], this.rows - 1) && isBetween(0, c[1], this.cols - 1)
    );
  };
  ...
  public around = (c: Coords, withDiagonals: boolean = false): Coords[] => {
    const res: Coords[] = [];

    const dirs = withDiagonals ? DIRS : BASIC_DIRS;

    dirs.forEach(([vDir, hDir]) => {
      const to = this.move(c, vDir, hDir);
  ...
  public dataAround = (c: Coords, withDiagonals: boolean = false): T[] => {
    return this.around(c, withDiagonals).map((c) => this.at(c));
  };
